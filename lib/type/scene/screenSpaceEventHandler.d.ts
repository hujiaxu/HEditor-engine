...
export type MouseDownEventFunction = <T extends SinglePositionEvent>(event: T) => void;
export type DoublePositionEventFunction = <T extends DoublePositionEvent>(event: T) => void;
...
export type EventFunctionType = WheelEventFunction | MouseDownEventFunction | DoublePositionEventFunction | PinchMoveEventFunction | MoveEventFunction;
export type InputActionFunction = (controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: Movement | PinchMovement | LastInertiaConstructor, rotationAxis?: Cartesian3) => void;
export type InputActionFunctionForEditor = (controller: ScreenSpaceCameraControllerForEditor, startPosition: Cartesian2, movement: Movement | PinchMovement | LastInertiaConstructor, rotationAxis?: Cartesian3) => void;
export interface EventTypeAndModifier {
    eventType: CameraEventType;
    modifier: KeyboardEventModifier;
}
export type LastInertiaType = '_lastInertiaSpinMovement' | '_lastInertiaZoomMovement' | '_lastInertiaTranslateMovement' | '_lastInertiaTiltMovement';
