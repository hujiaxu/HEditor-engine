...
QuarticRealPolynomial.computeRealRoots = (
  ...
  if (a === 0.0) {
    ...
    if (c === 0.0) {
      ...
      // x * (a * x^2 + c) = 0.
      roots = QuadraticRealPolynomial.computeRealRoots(a, 0, c) as number[]
    ...
    // x * (a * x^2 + b * x + c) = 0
    roots = QuadraticRealPolynomial.computeRealRoots(a, b, c) as number[]
...
function computeRealRoots(a: number, b: number, c: number, d: number) {
  const A = a
  const B = b / 3.0
  const C = c / 3.0
  const D = d
